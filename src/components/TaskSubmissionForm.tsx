  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage(error.error || 'Failed to submit task.')
      }
    } catch (error) {
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          ...
          <textarea
            ...
            placeholder="Describe your task here... (or select a template above)"
            className="w-full px-4 py-3 bg-gray-800/50 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6366f1] resize-none"
            rows={4}
            required
          />
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-gradient-to-r from-[#6366f1] to-[#8b5cf6] hover:from-[#5b5beb] hover:to-[#7c3aed] text-white font-semibold py-3 px-4 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
